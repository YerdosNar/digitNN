...
void init_layer(Layer *l, int pre_size, int size) {
    ...
    l->weights = (float*)malloc(pre_size *size * sizeof(float));
    l->weight_gradients = (float*)malloc(pre_size * size * sizeof(float));
...
exit(1);
    ...
    for(int i = 0; i < size; i++) {
        ...
        for(int j = 0; j < pre_size; j++) {
            ...
            l->weights[i * pre_size + j] = ((float)rand() / RAND_MAX) * 2 * limit - limit;
        }
    }
}
...
void feedforward(float input[], int size, Layer *l, int relu) {
    ...
    for(int i = 0; i < l->size; i++) {
        ...
        for(int j = 0; j < size; j++) {
            l->pre_train[i] += input[j] * l->weights[i * l->pre_size + j];
        }
        if(relu) {
...
void backpropagation(float input[], Layer **l, int lay_num, float target[]) {
    ...
    for(int k = lay_num - 2; k >= 0; k--) {
        for(int i = 0; i < l[k]->size; i++) {
            ...
            for(int j = 0; j < l[k+1]->size; j++) {
                l[k]->deltas[i] += l[k+1]->deltas[j] * l[k+1]->weights[j * l[k]->size + i];
            }
    ...
    for(int k = lay_num-1; k > 0; k--) {
        for(int i = 0; i < l[k]->size; i++) {
            ...
            for(int j = 0; j < l[k-1]->size; j++) {
                l[k]->weight_gradients[i * l[k-1]->size + j] += l[k]->deltas[i] * l[k-1]->neurons[j];
            }
        }
    }
    ...
    for(int i = 0; i < l[0]->size; i++) {
        ...
        for(int j = 0; j < l[0]->pre_size; j++) {
            l[0]->weight_gradients[i * l[0]->pre_size + j] += l[0]->deltas[i] * input[j];
        }
    }
}
...
void update_parameters(Layer *l, float learn_rate, int batch_size) {
    for(int i = 0; i < l->size; i++) {
        ...
        for(int j = 0; j < l->pre_size; j++) {
            l->weights[i * l->pre_size + j] -= (learn_rate * l->weight_gradients[i * l->pre_size + j]) / batch_size;
            l->weight_gradients[i * l->pre_size + j] = 0.0f;
        }
    ...
    for(int epoch = 0; epoch < epochs; epoch++) {
        ...
        for(int batch = 0; batch < num_images/batch_size; batch++) {
            ...
            for(int k = 0; k < lay_num; k++) {
                ...
                memset(l[k].weight_gradients, 0, l[k].pre_size * l[k].size * sizeof(float));
            }
